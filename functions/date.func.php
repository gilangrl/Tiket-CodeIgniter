...
//		$_date = $arrDate[0]."-".$arrDate[1]."-".$arrDate[2];
	...
	function getBulanIndo($month) {
		$arrMonth = array("Jan"=>"Januari", "Feb"=>"Februari", "Mar"=>"Maret", "Apr"=>"April", "May"=>"Mei", 
	...
	function getFormattedDate3($_date)
	{
		$arrMonth = array("1"=>"Januari", "2"=>"Februari", "3"=>"Maret", "4"=>"April", "5"=>"Mei", 
						  "6"=>"Juni", "7"=>"Juli", "8"=>"Agustus", "9"=>"September", "10"=>"Oktober", 
						  "11"=>"November", "12"=>"Desember");
	...
	function getFormattedDateTime3($_date)
	{
		...
		$date = $_date[1]." ".$_date[2]." ".$_date[4]." ".$_date[3];
		return $date;
	}

	function getTime($_time, $showTime=true)
	{
		if($_time < 10){
			$time = "0".$_time;
		} else {
	...
	// date input : database
	function getFormattedDate4($_date, $showTime=true)
	{
		$_date = explode(" ", $_date);
		$explodedDate = $_date[0];
		$explodedTime = $_date[1];
		...
		if($showTime == true)
			$datetime = $explodedDate;
		else
			$datetime = '<span style="white-space:nowrap">'.$explodedDate.'</span>';
